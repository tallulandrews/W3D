...
get_extreme_residuals <- function (dropout_obj, threshold = 0.1, labels=NA) {
	...
	if (is.na(labels)) {
		...
		palette = topo.colors(n=length(unique((colours))));
		X11()
		heatmap(log(as.matrix(data[sig.rows,])+1), ColSideColors = palette[colours], col=heatcolours)
	}
	return(table)
...
#		png("Monocle_Dropout_Plot_LabMeeting3.png", width=6,height=6, units="in", res=300)
		heatmap.2(heat_data, ColSideColors = palette[colours], col=heatcolours, scale="row",symbreaks=T, trace="none")
#		dev.off()
	}
	return(table)
}
